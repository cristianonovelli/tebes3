	...
	public Hashtable<String, Action> getTestAssertionRefHashtable(TAMLDOM tamlDOM, List<Input> inputs) {	
		...
		if ( externalTANodeList.getLength() > 0 ) {
			...
			for (int i = 0; i < externalTANodeList.getLength(); i++) {
				...
				Action taAction = new Action(i+1, name, Action.getReadyState(), Constants.TAML, Constants.TA, location, value, false, null);
	...
	public Hashtable<String, Action> getTestAssertionHashtable(TAMLDOM tamlDOM, List<Input> inputs) {
		...
		if ( taNodeList.getLength() > 0 ) {
			...
			for (int i = 0; i < taNodeList.getLength(); i++) {
				...
				Action taAction = new Action(i+1, name, Action.getReadyState(), Constants.TAML, Constants.TA, location, value, skip, description);
				taAction.setInputs(inputs);
				actionTable.put(value, taAction);
			}
		}
